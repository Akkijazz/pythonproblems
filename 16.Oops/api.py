...
def post_insert():
    ...
    cursor.execute(
        "INSERT into book_dir(book_id,book_name,author,genre) values(b.book_id,b.book_name,author,genre)"
    )
...
def put_update(book_id: int, updated_book: Book):
    ...
    cursor.execute(
        "UPDATE book_dir set book_id=updated_book[book_id], book_name=updated_book[book_name],author=updated_book[author],genre=updated_book[genre]"
    )
